...
fn part_two(input: &str, xr: (i32, i32), yr: (i32, i32)) -> usize {
    let mut s: HashSet<(i32, i32)> = HashSet::new();
    for y in -500..500  {
        for x in -500..500 {
            let t = run((0,0), (x,y), xr, yr);
            if t {
                s.insert((x,y));
                println!("{} {}", x, y);
            }
...
fn run(p: (i32, i32), v: (i32, i32), xr: (i32, i32), yr: (i32, i32)) -> bool {
    let mut p = p;
    let mut v = v;
    while p.0 < xr.1 && p.1 > yr.0 {
        p.0 += v.0;
        p.1 += v.1;
        v.0 += (-v.1).signum();
        v.1 -= 1;
    ...
    p.0 >= xr.0 && p.0 <= xr.1 && p.1 >= yr.0 && p.1 <= yr.1
}